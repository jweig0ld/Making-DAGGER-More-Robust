...
def policy_to_callable(
    ...
    if policy is None:
        ...
        def get_actions(states):
            ...
            # print(states)
            # log_probs = policy.get_distribution(states).log_prob(actions)
            # top_k_actions = th.topk(log_probs, k)[1] # Argmax returns the actions
            ...
            # frozen_outs = th.tensor([th.frozen_net(fut) for fut in fut_obs], requires_grad=False)
            # random_outs = th.tensor([th.random_net(fut) for fut in fut_obs])
            # mse = F.mse_loss(random_outs, frozen_outs) # We are expecting this to be (k x 1)
            # action = np.array([th.argmax(mse, dim=0)])
...
def generate_trajectories(
    ...
    *,
    ...
    # print("EXPERT ACT" , np.asarray(act_list).squeeze())
    return trajectories
