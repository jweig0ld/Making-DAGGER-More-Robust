...
@dataclasses.dataclass(frozen=True)
class RNDTrainingMetrics:
    """Container for the different components of behavior cloning loss."""
    rnd_loss: th.Tensor


@dataclasses.dataclass(frozen=True)
class RNDLossCalculator:
    """Functor to compute the loss used in RND."""
    ...
    def __call__(
        ...
        frozen_out = frozen_network(obs)
        online_out = online_network(obs)

        # print(online_out)

        # loss = th.mean(0.5*(frozen_out - online_out)**2)
        loss = F.mse_loss(online_out, frozen_out)

        return RNDTrainingMetrics(
            rnd_loss=loss,
        )
...
class BCLogger:
    ...
    def log_batch(
        ...
        rnd_metrics: RNDTrainingMetrics,
        ...
        # self._logger.record("rnd_loss", rnd_metrics.rnd_loss)
...
class BC(algo_base.DemonstrationAlgorithm):
    ...
    def __init__(
        ...
        *,
        ...
        frozen_net: th.nn.Sequential = None,
        online_net: th.nn.Sequential = None,
        ...
        self.optimizer = optimizer_cls(
            list(self.policy.parameters()) + list(self.online_net.parameters()),
            **optimizer_kwargs,
        )

        self.loss_calculator = BehaviorCloningLossCalculator(ent_weight, l2_weight)
        self.rnd_loss_calculator = RNDLossCalculator()
    ...
    def train(
        ...
        *,
        ...
        for (
            ...
            rnd_training_metrics = self.rnd_loss_calculator(self.frozen_net, self.online_net, obs)

            # Renormalise the loss to be averaged over the whole
            # batch size instead of the minibatch size.
            # If there is an incomplete batch, its gradients will be
            # smaller, which may be helpful for stability.
            loss = (training_metrics.loss + rnd_training_metrics.rnd_loss) * minibatch_size / self.batch_size
            loss.backward()
