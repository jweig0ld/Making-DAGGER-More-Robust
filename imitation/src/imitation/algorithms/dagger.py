...
class InteractiveTrajectoryCollector(vec_env.VecEnvWrapper):
    ...
    def __init__(
        ...
        save_dir: types.AnyPath,
        rng: np.random.Generator,
        frozen_net: th.nn.Sequential,
        online_net: th.nn.Sequential,
        k: int,
        bc_policy
        # online net frozen net and k  and bc policy instead of get robot acts function so we can get logits here
    ) -> None:
    ...
    def step_async(self, actions: np.ndarray) -> None:
        ...
        # print("ROBOT ACT ACTIONS LIST OF EXPERT", actual_acts)

        mask = self.rng.uniform(0, 1, size=(self.num_envs,)) > self.beta
        if np.sum(mask) != 0:
            # actual_acts[mask] = self.get_robot_acts(self._last_obs[mask])
            actions = th.tensor([0., 1., 2., 3.])
            # print(type(self._last_obs[mask]))
            log_probs = self.bc_policy.get_distribution(th.from_numpy(self._last_obs[mask])).log_prob(actions) # TODO: What is obs?
            top_k_actions = th.topk(log_probs, self.k)[1].squeeze() # Argmax returns the actions
            
            # Simulate taking the top-k actions in independent environments
            env_copies, fut_obs = [make_vec_env("LunarLander-v2", n_envs=1) for _ in range(self.k)], []
            for i in range(self.k):
                env_copies[i].reset()

            for past_action in self.act_list:
            ...
            for i in range(self.k):
                ...
                # candidate_action = np.array([top_k_actions])
                obs_prime, _, _, _ = env_copies[i].step(candidate_action)
                fut_obs.append(th.from_numpy(obs_prime))
            
            # Pass the observations through the frozen and unfrozen networks and pick
            # the one with the highest difference. 
            frozen_outs = th.tensor([self.frozen_net(fut) for fut in fut_obs], requires_grad=False).squeeze().unsqueeze(-1)
            random_outs = th.tensor([self.online_net(fut) for fut in fut_obs], requires_grad=False).squeeze().unsqueeze(-1)
            mse = F.mse_loss(random_outs, frozen_outs, reduction='none').squeeze() # We are expecting this to be (k,)
            k_idx = th.argmax(mse, dim=0)
            action = np.array([top_k_actions[k_idx]])
            # action = np.array([top_k_actions])
            # print('Action ', action)
            # print('Top K Actions ', top_k_actions)

            # actual_acts[mask] = equal to action we wanna take
            actual_acts[mask] = action

        self._last_user_actions = actions
        self.venv.step_async(actual_acts)
        # self.act_list.append(actual_acts.squeeze())
        self.act_list.append(actual_acts)
...
class DAggerTrainer(base.BaseImitationAlgorithm):
    ...
    def create_trajectory_collector(self) -> InteractiveTrajectoryCollector:
        ...
        collector = InteractiveTrajectoryCollector(
            ...
            save_dir=save_dir,
            rng=self.rng,
            frozen_net=self.bc_trainer.frozen_net,
            online_net=self.bc_trainer.online_net,
            k=2,
            bc_policy=self.bc_trainer.policy
        )
